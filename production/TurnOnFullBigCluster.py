...
for i, event in enumerate(t):
    ...
    for tob_num, tob in enumerate(tobs):
        ...
        l0_layer = ROOTClassDefs.Layer(l0_array, 3, 3, 2)
        l1_layer = ROOTClassDefs.Layer(l1_array, 12, 3, 2)
        l2_layer = ROOTClassDefs.Layer(l2_array, 12, 3, 2)
        l3_layer = ROOTClassDefs.Layer(l3_array, 3, 3, 2)
        had_layer = ROOTClassDefs.Layer(had_array, 3, 3, 2)
        ...
        print(found_seed_eta)

        # If no seed is found then go to next TOB
        if found_seed_eta is None:
            continue

        # If valid seed found, set it in the Event which recalculates seed_et and reco_et
